# CALSE 8
# AUX: PABLO DANIEL RIVAS MARROQUIN
...
class ErroresLexicos:
    def __init__(self, _token, _fila, _columna, _descripcion = ""):
        self._token = _token
        self._fila = _fila
        self._columna = _columna
        self._descripcion = _descripcion

    def getErrores(self):
        return {'token':self._token, 'fila':self._fila, 'columna':self._columna, 'descripcion':self._descripcion}
...
class Analizador:
    ...
    def Numero(self, _cadena : str):
        ...
        for i in tokens:
            try:
                ...
                # GUARDAR ERROR
                e = ErroresLexicos(i, self.linea, self.columna, "Error de codigo")
    ...
    def compile(self):
        ...
        if listaErroresLexicos:
            for i in listaErroresLexicos:
                i = i.getErrores()
                print(i['token'], " | ", i['fila'], ' | ', i['columna'], ' | ', i['descripcion'])

        file = open("d1.dot", "w", encoding='UTF-8')
        text = 'digraph { \n'
        text += 'nodo1[shape=plaintext ,label =<<TABLE><TR><TD>token</TD><TD>fila</TD><TD>columna</TD><TD>desc</TD></TR></TABLE>>] \n'
        text += '}'
        file.write(text)
        file.close()
